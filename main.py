...
#A debute le 7 septembre et a été terminer le 1 octobre 
...
def show_rules_of_the_game():
    ...
    print("Regles des combat:\nLors d'un combat, vous verrez ceci #####P########E####\nLa lettre P représente le joueur et le E représente l'ennemi\nLes # représente l'éspace libre")
    time.sleep(val_sleep)
    print("Lors d'un combat vous aurez plusieurs choix, vous pourrez avancer vers l'ennemi, reculer, attaquer ou prendre de la morphine\nVous déplacer vous permet de faire des choix stratégique comme fuire ou faire des attaquer au corps a corps\nSi vous attaquer vous aurez deux choix, le premier étant le corps a corps, le second étant d'uttiliser une arme a feux pour tirer sur l'ennemie\nNote: Pour effectuer le corps a corps, il faut au moins un # de distance entre l'ennemie et le joueur, finalment la morphine sers a reprendre de la vie")
    time.sleep(val_sleep)
    print("Note: Il est possible que vous trébucher ou que la morphine soit périmer, pouvant vous faire des degats")
...
def fight():
    ...
    print("Attention tu as devan toi un ", enemy.nom, "sauvage")
    #le mouvement
    while player.hp > 0 and enemy.hp > 0:
        ...
        if enemy.nom == "bob":
            ...
            if attaque == 0:
                print("tu est chanceux l'ennemi ne fais rien")
                ...
                if enemy.hp < 35 and player.hp > 50:
                    print("l'ennemi a peur et il recule")
                    num_ash_e_arr -= spd
                    num_ash_av += spd

                else:
                    if num_ash_av > 0:
                        print("l'ennemi recule")
                        num_ash_e_arr -= spd
                        num_ash_av += spd

            elif attaque == 3:
                if num_ash_av > 0:
                    print("l'ennemi avance")
                    num_ash_e_arr += spd
                    num_ash_av -= spd
        ...
        #attaque de joe
            ...
            if attaque == 0:
                ...
                if num_ash_av < 2:
                    print("quel malchance l'ennemi tataque avec ses grosse griffe\ntu te prend 15 de degat")
                    player.hp -= 15
